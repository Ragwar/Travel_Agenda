; (function () {
    // ——————————————————————————————
    // Read view-provided data from data-attributes
    // ——————————————————————————————
    const appEl = document.getElementById("activityLocationsApp");
    const apiKey = appEl.dataset.apikey;
    const CITY_PLACE_ID = appEl.dataset.placeid;
    const scheduleId = appEl.dataset.scheduleid;
    const scheduleStart = appEl.dataset.schedulestart;
    const scheduleEnd = appEl.dataset.scheduleend;
    const cityName = appEl.dataset.cityname;
    const savedActivities = JSON.parse(appEl.dataset.savedactivities);

    // ——————————————————————————————
    // Calendar configuration
    // ——————————————————————————————
    const slotHeight = 30;    // px per half-hour
    const fullStartHour = 1;     // 1 AM
    const fullEndHour = 25;    // covers 24:30
    const totalSlots = (fullEndHour - fullStartHour) * 2;
    const visibleSlots = 8;     // how many show in scroll
    let currentDate = new Date(scheduleStart);
    const scheduleEndDate = new Date(scheduleEnd);
    const events = [];
    const defaultDurations = {
        restaurant: 2, hotel: 1, park: 3, museum: 3, shopping_mall: 2
    };

    // DOM references
    let hourSlotsContainer, hourSlotsScroll, calendarHeader;
